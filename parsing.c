...
/* 	    "Function 'def' cannot define non-symbol '%s'!", */
/* 	    syms->cell[i]->sym); */
/*   } */
...
lval* builtin_min(lenv* e, lval* a) {
  for(int i = 0; i < a->count; i++) {
    LASSERT(a, a->cell[i]->type == LVAL_NUM,
	    "Function 'min' passed incorrect type for argument %i! "
...
lval* builtin_max(lenv* e, lval* a) {
  for(int i = 0; i < a->count; i++) {
    LASSERT(a, a->cell[i]->type == LVAL_NUM,
	    "Function 'max' passed incorrect type for argument %i! "
	    "Got %s, expected %s.", i, ltype_name(a->cell[i]->type),
	    ltype_name(LVAL_NUM));
  }
...
lval* builtin_op(lenv* e, lval* a, char *op) {
  ...
  while(a->count > 0) {
    ...
    if (strcmp(op, "%") == 0) {
      if(y->num == 0) {
	lval_del(x); lval_del(y);
	x = lval_err("Division by zerp!"); break;
      }
      x->num %= y->num;
    }
...
lval* builtin_len(lenv* e, lval* a) {
  LASSERT(a, a->count == 1,
	  "Function 'len' passed too many arugments! "
	  "Got %i, expected %i.", a->count, 1);
  LASSERT(a, a->cell[0]->type == LVAL_QEXPR,
...
lval* builtin_cons(lenv* e, lval* a) {
  ...
  lval* r = lval_qexpr();
  r = lval_add(r, lval_pop(a,0));

  for(int i=0; i <= a->cell[0]->count; i++) {
    r = lval_add(r, lval_pop(a->cell[0], 0));
  }
...
lval* builtin_eval(lenv* e, lval* a) {
  LASSERT(a, a->count == 1,
	  ...
	  "Got %i, expected %i.", a->count, 1);
  LASSERT(a, a->cell[0]->type == LVAL_QEXPR,
	  "Function 'eval' passed incorrect type for arugment 0! "
	  "Got %s, expected %s.",
	  ltype_name(a->cell[0]->type), ltype_name(LVAL_QEXPR));
...
int main(int argc, char** argv) {
  ...
  while (1) {
    ...
    if (mpc_parse("<stdin>", input, Lispy, &r)) {
      result = lval_eval(e, lval_read(r.output));
      lval_println(result);
      lval_del(result);
